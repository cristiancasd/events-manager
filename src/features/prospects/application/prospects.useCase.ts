...
import { ProspectEntity, ProspectRepository, ProspectUseCaseInterface, ProspectValue } from "../domain";
...
export class ProspectsUseCase implements ProspectUseCaseInterface {
  ...
  async validateDuplicatedData(
    ...
    if (phone != null) {
      try {
        await this._prospectRepository.findProspectByPhone(commerceUserId,phone);
        phoneFound = true;
      } catch (err) {
  ...
  async createProspect(data: ProspectEntity ): Promise<ProspectEntity> {
    return await this._prospectRepository.createProspect(
      data
    );
  }
  ...
  async findProspectByPhone(
    commerceUid: string, phone: string,
  ): Promise<ProspectEntity> {
  return  await this._prospectRepository.findProspectByPhone(commerceUid,phone);
  }
  ...
  async findProspectsByUserCommerce(userCommerceUid: string): Promise<ProspectEntity[]> {
    return  await this._prospectRepository.findProspectsByUserCommerce(userCommerceUid);
  }
