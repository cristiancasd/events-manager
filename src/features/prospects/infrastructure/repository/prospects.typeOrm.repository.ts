...
import { BadRequestError, DataBaseError, NotFoundError, codeProspectNotFound, commerceIdInvalidMessage, errorHandlerTypeOrm, errorMessageProspectNotFound, errorMsgDb, userCommerceInvalidMessage } from "../../../../core";
import { connectDB } from "../../../../database";
import { UserUseCase } from "../../../user";
import { ProspectEntity, ProspectRepository, ProspectValue } from "../../domain";
...
export class ProspectsTypeORMRepository implements ProspectRepository {
  ...
  async createProspect(
    ...
    const prospectRepository = connectDB.getRepository(
      ProspectTypeORMEntity
    );

  
    const userCommerce = await this.userCommerceUseCase.findUserByUid(data.userCommerceUid);

    if ( userCommerce.commerceUid != data.commerceUid)
      throw new BadRequestError(errorMessageProspectNotFound, codeProspectNotFound);
    ...
    // Save on DB
    const prospectSaved: ProspectTypeORMEntity =
      await prospectRepository.save({
        ...prospect,
        userCommerce,
      });
  ...
  @errorHandlerTypeOrm
  async findProspectByPhone(commerceUid: string, phone: string): Promise<ProspectEntity> {
    ...
    return new ProspectValue({...prospect, commerceUid: prospect.userCommerce.commerce.id, userCommerceUid: prospect.userCommerce.id});
  }

  @errorHandlerTypeOrm
  async findProspectsByUserCommerce(userCommerceUid: string): Promise<ProspectEntity[]> {
    const prospectRepository = connectDB.getRepository(ProspectTypeORMEntity);
    
    const queryBuilder = prospectRepository
    .createQueryBuilder('prospect')
    .where('prospect.userCommerce.id = :userCommerceUid', { userCommerceUid });
