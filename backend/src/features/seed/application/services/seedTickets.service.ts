...
export const seedTicketsService = async (
  ...
  try {
    ...
    ticketsFound.map((data) => {
      if (data.name.toLocaleLowerCase() == ticket1.name.toLocaleLowerCase())
        ticketUid1 = data.id;
      if (data.name.toLocaleLowerCase() == ticket2.name.toLocaleLowerCase())
        ticketUid2 = data.id;
    });
