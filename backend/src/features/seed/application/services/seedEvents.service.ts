...
export const seedEventsService = async (
  ...
  try {
    ...
    eventsFound.map((data) => {
      if (data.name.toLocaleLowerCase() == event1.name.toLocaleLowerCase())
        eventUid1 = data.id;
      if (data.name.toLocaleLowerCase() == event2.name.toLocaleLowerCase())
        eventUid2 = data.id;
    });
